    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box rounded bg-white p-3">
                ...
                <hr>
                <form action="{{ route('result_piutang_pelanggan') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-md-3">
            ...
            <div class="box bg-white rounded p-5 mt-3" name="ticket2" id="ticket2">
                <div class="row">
                    <div class="col-md-12 text-center">
                        ...
                        <h2 class="font-weight-bolder">Lapporan Piutang Pelanggan</h2>
                ...
                <table class="table mt-5" style="width:100%" id="example">
                    ...
                    <tbody>
                        @forelse ($penjualan as $item)
                            ...
                            @php
                                $get = \App\Models\Finance\Penjualan::query()->where(['creat_by_company' => Auth::user()->pegawai->company->id])->where(['customer_id' => $item->id])->whereBetween('tanggal_transaksi', [$tanggal_awal, $tanggal_akhir])->with(['customer'])->get();
                                $get_total = \App\Models\Finance\Penjualan::query()->where(['creat_by_company' => Auth::user()->pegawai->company->id])->where(['customer_id' => $item->id])->whereBetween('tanggal_transaksi', [$tanggal_awal, $tanggal_akhir])->with(['customer'])->sum('total');
                                $get_piutang = \App\Models\Finance\Penjualan::query()->where(['creat_by_company' => Auth::user()->pegawai->company->id])->where(['customer_id' => $item->id])->whereBetween('tanggal_transaksi', [$tanggal_awal, $tanggal_akhir])->with(['customer'])->sum('sisa_tagihan');
